<template>
  ...
  <div v-if="showLegalDialog" class="legal-dialog-overlay">
    <div class="legal-dialog">
      ...
      <div class="dialog-buttons">
        <button class="cancel-button" @click="cancelSave">取消</button>
  ...
  <div class="container">
      ...
      <div class="disclaimer-content">
        ...
        <div class="warning-text">
          ...
          <p>
            ...
            3.
            因违法使用本项目造成的任何法律责任和损失，需自行承担，与本项目无关。<br />
            4. 如果使用本项目请遵守相关法律法规。
          </p>
        </div>
      </div>
    ...
    <div ref="editorControls" class="editor-controls">
      <!-- 顶部固定按钮 -->
      <div
        class="button-group"
        style="
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('basic')">
          <h3>印章基本设置</h3>
          <span class="expand-icon" :class="{ expanded: expandedGroups.basic }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.basic" class="group-content">
          <label class="checkbox-label">
            <input v-model="isCircleDetect" type="checkbox" />
            提取圆形印章
          </label>
          <label
            >印章宽度 (mm):
            <input
              v-model.number="drawStampWidth"
              type="number"
              min="1"
              max="50"
              step="1"
          ...
          <label
            >印章高度 (mm):
            <input
              v-model.number="drawStampHeight"
              type="number"
              min="1"
              max="50"
              step="1"
          ...
          <label
            >圆形边框宽度 (mm):
            <input v-model.number="circleBorderWidth" type="number" step="0.1"
          /></label>
          <label>印章颜色: <input v-model="primaryColor" type="color" /></label>
        </div>
      </div>
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('company')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.company }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.company" class="group-content">
          <div
          ...
          >
            ...
            <label>
              字体大小 (mm):
              <input
                v-model.number="company.fontHeight"
                type="number"
                min="1"
                max="10"
                step="0.1"
            ...
            <label>
              边距 (mm):
              <input
                v-model.number="company.borderOffset"
                type="number"
                min="-10"
                max="10"
                step="0.05"
            ...
            <label class="checkbox-label">
              <input v-model="company.adjustEllipseText" type="checkbox" />
              调整椭圆文字
            </label>
            ...
            <label>
              ...
              <span
                >{{
                  company.startAngle
                    ? ((company.startAngle * 180) / Math.PI).toFixed(0)
                    : 0
                }}°</span
              >
            </label>
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('stampType')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.stampType }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.stampType" class="group-content">
          <div
            v-for="(type, index) in stampTypeList"
            :key="index"
            class="stamp-type-item"
          >
            ...
            <label>
              文字内容:
              <input v-model="type.stampType" type="text" />
            </label>
            <label>
              字体大小 (mm):
              <input
                v-model.number="type.fontHeight"
                type="number"
                min="1"
                max="10"
                step="0.1"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('code')">
          <h3>印章编码设置</h3>
          <span class="expand-icon" :class="{ expanded: expandedGroups.code }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.code" class="group-content">
          ...
          <label
            >字体大小 (mm):
            <input v-model.number="codeFontSizeMM" type="number" step="0.1"
          /></label>
          <label>
            字体粗细:
          ...
          <label>
            ...
            <input
              v-model.number="codeCompression"
              type="range"
              min="0.0"
              max="3"
              step="0.01"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('taxNumber')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.taxNumber }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.taxNumber" class="group-content">
          ...
          <label>
            ...
            <input
              v-model.number="taxNumberPositionY"
              type="range"
              min="-10"
              max="10"
              step="0.1"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('images')">
          <h3>图片列表设置</h3>
          <span class="expand-icon" :class="{ expanded: expandedGroups.images }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.images" class="group-content">
          <div class="image-list">
            ...
            >
              ...
              <div v-if="image.imageUrl" class="image-preview">
              ...
              <label>
                图片宽度 (mm):
                <input
                  v-model.number="image.imageWidth"
                  type="number"
                  min="1"
                  max="100"
                  step="0.5"
              ...
              <label>
                图片高度 (mm):
                <input
                  v-model.number="image.imageHeight"
                  type="number"
                  min="1"
                  max="100"
                  step="0.5"
              ...
              <label>
                水平位置 (mm):
                <input
                  v-model.number="image.positionX"
                  type="number"
                  min="-20"
                  max="20"
                  step="0.5"
              ...
              <label>
                垂直位置 (mm):
                <input
                  v-model.number="image.positionY"
                  type="number"
                  min="-20"
                  max="20"
                  step="0.5"
              ...
              <label class="checkbox-label">
                <input v-model="image.keepAspectRatio" type="checkbox" />
                保持宽高比
              </label>
            </div>
          </div>
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('star')">
          <h3>五角星设置</h3>
          <span class="expand-icon" :class="{ expanded: expandedGroups.star }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.star" class="group-content">
          <label class="checkbox-label">
            <input v-model="shouldDrawStar" type="checkbox" />
            绘制五角星
          </label>
          <div v-if="shouldDrawStar">
            <label>
              五角星直径 (mm):
              <input v-model.number="starDiameter" type="number" step="0.1" />
            </label>
            <label>
              垂直位置 (mm):
              <input
                v-model.number="starPositionY"
                type="number"
                min="-10"
                max="10"
                step="0.1"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('security')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.security }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.security" class="group-content">
          <label>
            启用防伪纹路:
            <input v-model="securityPatternEnabled" type="checkbox" />
          </label>
          <button @click="drawStamp(true, false)">刷新纹路</button>
          <label
            >纹路数量:
            <input
              v-model.number="securityPatternCount"
              type="range"
              min="1"
              max="100"
              step="1"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('roughEdge')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.roughEdge }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.roughEdge" class="group-content">
          <label class="checkbox-label">
            <input v-model="shouldDrawRoughEdge" type="checkbox" />
            启用毛边效果
          </label>
          <label v-if="shouldDrawRoughEdge">
            毛边宽度 (mm):
            <input
              v-model.number="roughEdgeWidth"
              type="range"
              min="0.05"
              max="0.5"
              step="0.05"
          ...
          <label v-if="shouldDrawRoughEdge">
            毛边高度 (mm):
            <input
              v-model.number="roughEdgeHeight"
              type="range"
              min="0.1"
              max="5"
              step="0.1"
          ...
          <label v-if="shouldDrawRoughEdge">
            毛边概率:
            <input
              v-model.number="roughEdgeProbability"
              type="range"
              min="0"
              max="1"
              step="0.01"
          ...
          <label v-if="shouldDrawRoughEdge">
            毛边点数:
            <input
              v-model.number="roughEdgePoints"
              type="range"
              min="100"
              max="1000"
              step="10"
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('aging')">
          <h3>做旧效果</h3>
          <span class="expand-icon" :class="{ expanded: expandedGroups.aging }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.aging" class="group-content">
          <label class="checkbox-label">
            <input v-model="applyAging" type="checkbox" />
            启用做旧效果
          </label>
          <label class="checkbox-label">
            <input v-model="manualAging" type="checkbox" />
            手动做旧
          </label>
      ...
      <div class="control-group">
        <div class="group-header" @click="toggleGroup('innerCircle')">
          ...
          <span
            class="expand-icon"
            :class="{ expanded: expandedGroups.innerCircle }"
            >▼</span
          >
        </div>
        <div v-show="expandedGroups.innerCircle" class="group-content">
          <button @click="addNewInnerCircle">添加新行</button>
          <div
            v-for="(innerCircle, index) in innerCircleList"
            :key="index"
            class="inner-circle-item"
          >
            ...
            <label>
              内圈圆线宽 (mm):
              <input
                v-model.number="innerCircle.innerCircleLineWidth"
                type="number"
                min="0.05"
                max="0.5"
                step="0.05"
            ...
            <label>
              内圈圆半径X (mm):
              <input
                v-model.number="innerCircle.innerCircleLineRadiusX"
                type="number"
                min="1"
                max="50"
                step="0.1"
            ...
            <label>
              内圈圆半径Y (mm):
              <input
                v-model.number="innerCircle.innerCircleLineRadiusY"
                type="number"
                min="1"
                max="50"
                step="0.1"
    ...
    <div class="canvas-container">
      <div
        style="display: flex; flex-direction: row; margin-top: 12px; gap: 12px"
      >
        <!-- 做旧效果设置 -->
        <div class="control-group">
          <h3>做旧效果</h3>
      ...
      <canvas ref="stampCanvas" width="600" height="600" />
    </div>
...
const restoreDrawConfigs = () => {
  ...
  // 防伪纹路
  securityPatternEnabled.value =
    drawConfigs.securityPattern.openSecurityPattern;
...
const updateFontPreview = (event: Event) => {
  ...
  if (element.tagName === "INPUT") {
    const select = element.parentElement?.querySelector(
      ".font-select"
...
const loadDefaultTemplate = (template: Template) => {
  try {
    ...
    currentTemplateIndex.value =
      -1 - defaultTemplates.findIndex(t => t === template);
  } catch (error) {
